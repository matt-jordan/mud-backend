...
function createLogger () {
  const { level, pretty, filePath: path }: {level: string, pretty: boolean, filePath: string} = config.get('log');

  if (level === 'silent') {
    return bunyan.createLogger({ name, streams: [] });
  }

  const bunyanLevel = (level as any) as bunyan.LogLevelString;
...
const { logUncaughtException = undefined }: { logUncaughtException: boolean | undefined } = config.get('log');
