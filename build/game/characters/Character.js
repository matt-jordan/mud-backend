...
const ActionEffectQueue_js_1 = __importDefault(require("./helpers/ActionEffectQueue.js"));
const characterDetails_js_1 = __importDefault(require("./helpers/characterDetails.js"));
const Conversation_js_1 = __importDefault(require("./helpers/Conversation.js"));
const CurrencyManager_js_1 = __importDefault(require("./helpers/CurrencyManager.js"));
const FactionManager_js_1 = __importDefault(require("./helpers/FactionManager.js"));
const Quest_js_1 = __importDefault(require("./quests/Quest.js"));
...
const QuestModel_js_1 = __importDefault(require("../../db/models/QuestModel.js"));
const Fighter_js_1 = __importDefault(require("../classes/Fighter.js"));
const Priest_js_1 = __importDefault(require("../classes/Priest.js"));
const Rogue_js_1 = __importDefault(require("../classes/Rogue.js"));
const Mage_js_1 = __importDefault(require("../classes/Mage.js"));
const interpreter_js_1 = require("../language/interpreter.js");
const CommandSet_js_1 = require("../commands/CommandSet.js");
const Error_js_1 = require("../commands/default/Error.js");
const ObjectContainer_js_1 = require("../ObjectContainer.js");
const inanimates_js_1 = require("../objects/inanimates.js");
const corpses_js_1 = __importDefault(require("../objects/factories/corpses.js"));
 ...
 */
class Character extends events_1.default {
    /**
    ...
    constructor(model, world) {
        ...
        this.world = world;
        this.commandSets = [CommandSet_js_1.AdminCommandSet, CommandSet_js_1.DefaultCommandSet, CommandSet_js_1.SocialCommandSet];
        this.name = 'Unknown';
        this.description = '';
        this.gender = '';
        this.classes = [];
        ...
        this.factions = new FactionManager_js_1.default(this);
        this.questsCompleted = [];
        this.questsGiven = [];
        this.currencies = new CurrencyManager_js_1.default();
        this.skills = new Map();
        // Add default skills
        this.skills.set('scholar', 0);
        this.skills.set('observation', 0);
        this.skillDice = new DiceBag_js_1.default(1, 100, 4);
        this.attackActions = new ActionEffectQueue_js_1.default();
        this.effects = new ActionEffectQueue_js_1.default();
    ...
    set transport(_transport) {
        ...
        if (_transport) {
            log_js_1.default.debug({ characterId: this.id }, 'Associating transport to character');
        }
        else {
            log_js_1.default.debug({ characterId: this.id }, 'Removing transport from character');
        }
        ...
        this._transport.on('disconnect', () => {
            log_js_1.default.debug({ characterId: this.id }, 'Disconnect event received; dis-associating from character');
            this._transport = null;
        });
        this._transport.on('message', async (message) => {
            try {
                ...
                if (!rcvMessage.messageType) {
                    log_js_1.default.debug({ rcvMessage }, 'No messageType');
                    return;
                }
                ...
                this.commandSets.forEach((commandSet) => {
                    ...
                    if (command) {
                        log_js_1.default.debug({ characterId: this.id, command: rcvMessage.messageType }, 'Generated command');
                        generatedCommands.push(command);
                    }
                });
                if (generatedCommands.length > 0) {
                    await (0, asyncForEach_js_1.default)(generatedCommands, async (command) => {
                        await command.execute(this);
                    });
                }
                else {
                    const errorAction = CommandSet_js_1.DefaultCommandSet.commands[Error_js_1.ErrorFactory.name]
                        .generate(rcvMessage.messageType, rcvMessage.parameters);
                    await errorAction.execute(this);
                }
            }
            catch (e) {
                log_js_1.default.warn({ message: e.message }, 'Error');
            }
        });
    }
    /**
    ...
    addExperience(encounterLevel, partySize = 1) {
        if (this.classes.length === 0) {
            log_js_1.default.debug({ characterId: this.id }, `${this.toShortText()}: ignoring experience gain due to no character class`);
            return;
        }
        const minLevel = this.classes.map((characterClass) => characterClass.level).reduce((a, b) => Math.min(a, b));
        if (!minLevel) {
            log_js_1.default.debug({ characterId: this.id }, `${this.toShortText()}: ignoring experience gain due to class with unknown level`);
            return;
        }
     ...
     */
    async _handleDeath() {
        log_js_1.default.debug({ characterId: this.id }, 'Handling death for character');
        if (this.transport) {
        ...
        if (this.room) {
            ...
            const corpse = await (0, corpses_js_1.default)(this);
            if (corpse) {
                ...
                await (0, asyncForEach_js_1.default)(currencies, async (currencyDef) => {
                    const currency = await (0, currency_js_1.default)(currencyDef);
        ...
        await (0, asyncForEach_js_1.default)(this.questsGiven, async (quest) => {
            await quest.destroy();
            Quest_js_1.default.unregister(quest);
        });
        ...
        if (this.model.accountId) {
            ...
            await CharacterModel_js_1.default.deleteOne({ _id: this.model._id });
            this.model = null;
        }
    }
    /**
    ...
    toLongText(observer = null) {
        ...
        description += (0, characterDetails_js_1.default)(this, observer);
        return `${this.name}\n${description}`;
    }
    /**
    ...
    findItemsOnCharacter(name, location = null) {
        ...
        locations.forEach((location) => {
            if (this.physicalLocations[location]
                ...
                && (0, ObjectContainer_js_1.objectNameComparitor)(name, this.physicalLocations[location].item.name)) {
                items.push({
                    location,
    ...
    removeItemOnCharacter(name, location) {
        ...
        log_js_1.default.debug({ characterId: this.id, itemId: item.id }, `${this.name} is no longer wearing ${item.name} on ${location}`);
        return item;
    }
    /**
    ...
    addHauledItem(item) {
        ...
        log_js_1.default.debug({ characterId: this.id, itemId: item.id }, `${this.name} is now hauling ${item.name}`);
    ...
    removeHauledItem(_item) {
        ...
        this.carryWeight -= item.weight;
        log_js_1.default.debug({ characterId: this.id, itemId: item.id }, `${this.name} is no longer hauling ${item.name}`);
        return true;
    }
    /**
    ...
    sendImmediate(message) {
        ...
        log_js_1.default.debug({ characterId: this.id, message: jsonMessage }, 'Sending message');
    ...
    moveToRoom(room) {
        ...
        log_js_1.default.debug({ characterId: this.id, roomId: room.id }, 'Moving to room');
        this.room = room;
        ...
        const new_sub = this.mb.subscribe(this.room.id, (packet) => {
            ...
            if (packet.senders && packet.senders.includes(this.id)) {
                if (!packet.options || !packet.options.sendToSelf) {
                    log_js_1.default.debug({ characterId: this.id }, 'Suppressing message to self');
                    return;
                }
            }
            ...
            if (typeof message === 'object' && message.socialType) {
                const textMessage = (0, interpreter_js_1.interpretLanguage)(message.language, this, message.text);
    ...
    onTick() {
        ...
        expiredEffects.forEach((effect) => {
            log_js_1.default.debug({ characterId: this.id, effect }, 'Effect expired off of character');
        });
        if (this.currentState !== Character.STATE.FIGHTING) {
            ...
            if (this.attributes.energypoints.current < this.attributes.energypoints.base) {
                ...
                this.attributes.energypoints.current = Math.min(this.attributes.energypoints.current + energyRegen, this.attributes.energypoints.base);
            }
            if (this.attributes.hitpoints.current < this.attributes.hitpoints.base) {
                ...
                this.attributes.hitpoints.current = Math.min(this.attributes.hitpoints.current + hitpointRegen, this.attributes.hitpoints.base);
            }
            if (this.attributes.manapoints.current < this.attributes.manapoints.base) {
                ...
                this.attributes.manapoints.current = Math.min(this.attributes.manapoints.current + manaRegen, this.attributes.manapoints.base);
            }
    ...
    async load(loadSet = null) {
        log_js_1.default.debug({ characterId: this.model.id, characterName: this.model.name, loadSet }, 'Loading character');
        if (!loadSet) {
            ...
            this.classes = this.model.classes.map((characterClass) => {
                ...
                switch (characterClass.type) {
                    case 'fighter':
                        _class = new Fighter_js_1.default(this);
                        _class.experience = characterClass.experience;
                        _class.level = characterClass.level;
                        break;
                    case 'priest':
                        _class = new Priest_js_1.default(this);
                        _class.experience = characterClass.experience;
                        _class.level = characterClass.level;
                        break;
                    case 'mage':
                        _class = new Mage_js_1.default(this);
                        _class.experience = characterClass.experience;
                        _class.level = characterClass.level;
                        break;
                    case 'rogue':
                        _class = new Rogue_js_1.default(this);
                        _class.experience = characterClass.experience;
                        _class.level = characterClass.level;
                        break;
                    default:
                        log_js_1.default.warn({ characterId: this.model._id.toString() }, `Unknown character class: ${characterClass.type}`);
                }
                return _class;
            }).filter(c => c);
            ...
            this.attributes.manapoints.regen = Math.max(this.getAttributeModifier('intelligence'), this.getAttributeModifier('wisdom'), 1);
            this.attributes.energypoints.regen = 5 + this.getAttributeModifier('constitution');
            await (0, asyncForEach_js_1.default)(Character.physicalLocations, async (physicalLocation) => {
                if (this.model.physicalLocations[physicalLocation]) {
                    const modelDef = this.model.physicalLocations[physicalLocation].item;
                    if (modelDef) {
                        const item = await (0, inanimates_js_1.loadInanimate)(modelDef);
                        this.carryWeight += item.weight;
            ...
            if (this.model.conversationId) {
                const conversationModel = await ConversationModel_js_1.default.findById(this.model.conversationId);
                if (!conversationModel) {
                    log_js_1.default.warn({ characterId: this.model._id.toString(), conversationId: this.model.conversationId.toString() }, 'Unable to find conversation for character');
                }
                else {
                    this.conversation = new Conversation_js_1.default(conversationModel, this);
                    await this.conversation.load();
                }
            }
            if (this.model.factions) {
                await (0, asyncForEach_js_1.default)(this.model.factions, async (factionDef) => {
            ...
            const questModels = await QuestModel_js_1.default.findByQuestGiver(this.model.characterRef);
            if (questModels) {
                await (0, asyncForEach_js_1.default)(questModels, async (questModel) => {
                    const quest = new Quest_js_1.default(questModel, this);
                    await quest.load();
                    Quest_js_1.default.register(quest);
            ...
            if (this.model.roomId) {
                ...
                roomId = config_1.default.game.defaultRoomId;
            }
            ...
            const quests = Quest_js_1.default.activeQuests(this);
            if (quests) {
            ...
            log_js_1.default.error({ loadSet, characterId: this.id }, 'Unknown load set when loading character');
        }
    }
    /**
     * Save the character
    ...
    async save() {
        ...
        (0, asyncForEach_js_1.default)(this.questsGiven, async (quest) => {
            await quest.save();
        });
        await this.model.save();
    }
