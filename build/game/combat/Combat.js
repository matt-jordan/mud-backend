    ...
    processRound() {
        ...
        for (let i = 0; i < attacks.length; i += 1) {
            ...
            if (attackRoll <= defenseCheck) {
                ...
                this.attacker.room.sendImmediate([this.attacker, this.defender,], this.combatMessage(`${this.attacker.toShortText()} attempts to ${attack.verbs.firstPerson} ${this.defender.toShortText()} ${attack.name ? `with their ${attack.name} ` : ''}in their ${hitLocation} but misses!`));
                continue;
            }
            // Blocking
            ...
            if (shield) {
                ...
                if (attackRoll <= blockRoll) {
                    ...
                    this.attacker.room.sendImmediate([this.attacker, this.defender,], this.combatMessage(`${this.attacker.toShortText()} attempts to ${attack.verbs.firstPerson} ${this.defender.toShortText()} ${attack.name ? `with their ${attack.name} ` : ''}in their ${hitLocation} but ${this.defender.toShortText()} blocks it with their ${shield.name}!`));
                    continue;
                }
                else {
                    log.debug({
            ...
            if (this.defender.attributes.hitpoints.current === 0) {
                ...
                this.attacker.room.sendImmediate([this.attacker, this.defender,], this.combatMessage(`${this.attacker.toShortText()} has killed ${this.defender.toShortText()}`));
                return Combat.RESULT.DEFENDER_DEAD;
            }
        }
        this._round += 1;
